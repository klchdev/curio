...
export const landing = {
  ...
  step2Text: `Написанное на страницах игр в Steam переносится одной кнопкой, остальное дописывается руками. Хватает ${THRESHOLDS.MIN_REVIEWS_TO_RUN} отзывов, чтобы получить первый разбор, а к ${THRESHOLDS.MIN_REVIEWS_FOR_CONFIDENCE} он становится уверенным.`,
...
export const onboarding = {
  ...
  keyTitle: "Ключ от нейросети — необязательно",
  keyText:
    "Без ключа советы всё равно будут: их посчитает алгоритм по жанрам и твоим тирам. Ключ нужен, чтобы Кьюрио читал сами тексты отзывов — тогда появляются разборы, вердикты и объяснения твоими же словами. Сервис своего ключа не держит, у Gemini есть бесплатный тариф.",
  ...
  readyNoKey:
    "Ключ так и не добавлен — это нормально: советы посчитает алгоритм по жанрам. Захочешь разбор по своим словам — ключ ждёт в настройках.",
