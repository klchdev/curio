...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  if (!Number.isInteger(entryId)) return json({ error: s.errors.noEntry }, 400);

  if (body.action === "remove") {
    const tagId = Number(body.tagId);
    if (!Number.isInteger(tagId)) return json({ error: s.errors.noTag }, 400);
    const ok = await removeEntryTag(userId, entryId, tagId);
    return ok ? json({ ok: true }) : json({ error: s.errors.entryNotFound }, 404);
  }

  const label = String(body.label ?? "").trim();
  const kind = body.kind === "praise" ? "praise" : "complaint";
  if (label.length < 2 || label.length > 60) return json({ error: s.errors.badTagLabel }, 400);

  const tag = await addEntryTag(userId, entryId, kind, label);
  return tag ? json({ ok: true, tag }) : json({ error: s.errors.entryNotFound }, 404);
};
